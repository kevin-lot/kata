...
fn main() {
    ...
    let rover = match Rover::new(
        Position::new(0, 0, Orientation::North),
        Planet::new((10, 10), vec![Rock::new(1, 1)]),
    ) {
        Ok(r) => r,
    ...
    let (rover, collisions) = servo_command.move_rover(commands);
    ...
    let rover_x = rover.get_position().x;
    let rover_y = rover.get_position().y;
    ...
    for y in (0..size_y).rev() {
        for x in 0..size_x {
            ...
            if rocks.contains(&Rock::new(x, y)) {
                print!("X");
                continue;
            }
            print!(".");
