...
sealed class PokerHand {
    ...
    data class Straight(
        ...
        companion object {
            val aceLow =
                setOf(CardStrength.TWO, CardStrength.THREE, CardStrength.FOUR, CardStrength.FIVE, CardStrength.ACE)
        }

        init {
            val sortedCards = cards.toList().sortedBy { it.strength.ordinal }
            val isRegular = sortedCards.zipWithNext().all { (a, b) -> b.strength.ordinal == a.strength.ordinal + 1 }
            val isAceLow = sortedCards.map { it.strength }.toSet() == aceLow
            require(isRegular || isAceLow) { "Cards are not consecutive" }
        }
    }
