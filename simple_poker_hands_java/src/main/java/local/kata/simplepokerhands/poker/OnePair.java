package local.kata.simplepokerhands.poker;

import local.kata.simplepokerhands.cards.Card;
import local.kata.simplepokerhands.tuples.Pair;

public final class OnePair extends PokerHand {
    public final Pair<Card, Card> cards;
    public final Card kicker;

    public OnePair(Pair<Card, Card> cards, Card kicker) {
        this.cards = cards;
        this.kicker = kicker;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        OnePair onePair = (OnePair) o;
        return java.util.Objects.equals(cards, onePair.cards) && java.util.Objects.equals(kicker, onePair.kicker);
    }

    @Override
    public int hashCode() {
        return java.util.Objects.hash(cards, kicker);
    }
}
