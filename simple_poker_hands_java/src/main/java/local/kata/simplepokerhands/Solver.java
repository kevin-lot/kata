...
import static local.kata.simplepokerhands.poker.Straight.aceLow;
...
public final class Solver {
    ...
    public static Optional<Straight> findStraight(List<Card> cards) {
        ...
        if (strengths.get(4).ordinal() - strengths.get(0).ordinal() == 4) {
        ...
        if (Set.copyOf(strengths).equals(aceLow)) {
            return Optional.of(new Straight(new Quintuple<>(
                sortedCards.get(4),
                sortedCards.get(0),
                sortedCards.get(1),
    ...
    public static Optional<FullHouse> findFullHouse(List<Card> cards) {
        ...
        var groups = cards.stream().collect(groupingBy(Card::strength));
    ...
    private static Optional<List<Card>> findSameCards(List<Card> cards, int howMany) {
        var groupsByStrength = cards.stream().collect(groupingBy(Card::strength));

        return groupsByStrength.values().stream()
            .filter(g -> g.size() == howMany)
            .max(Comparator.comparingInt(g -> g.getFirst().strength().ordinal()));
    }
}
